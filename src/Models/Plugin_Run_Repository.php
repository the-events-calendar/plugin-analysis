...
class Plugin_Run_Repository extends Repository implements Contracts\Deletable, Contracts\Insertable, Contracts\Updatable {
	/**
	 * {@inheritDoc}
	 */
	public function delete( ModelContract $model ): bool {
		// @todo schema?
		return (bool) DB::delete( pperf_get_plugin_run_table_name(), [ 'perf_run_id' => $model->perf_run_id ], [ '%d' ] );
	}
	...
	public function insert(  ModelContract $model ):  ModelContract {
		// @todo schema?
		DB::insert( pperf_get_plugin_run_table_name(), $model->toArray(), array_fill(0,count($model->toArray()),'%s') );
		$model->perf_plugin_run_id = DB::last_insert_id();
	...
	function prepareQuery(): ModelQueryBuilder {
		...
		return $builder->from( pperf_get_plugin_run_table_name() );
	}
	...
	public function update(  ModelContract $model ):  ModelContract {
		DB::update( pperf_get_plugin_run_table_name(), $model->toArray(), [ 'perf_run_id' => $model->perf_run_id ], array_fill(0,count($model->toArray()),'%s'), [ '%d' ] );
