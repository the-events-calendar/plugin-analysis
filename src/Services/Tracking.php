...
class Tracking {
	...
	public function on_shutdown() {
		...
		// First run on snapshot?
		if ( ! Snapshot::find( $hash ) ) {
			$snapshot                       = new Snapshot();
			$snapshot->active_plugins       = $cereal;
			$snapshot->plugins_version_hash = $hash;
			$snapshot->created_datetime     = $now;
			$snapshot->save();
		}

		// Save this requests run.
		$run->plugins_version_hash = $hash;
		$run->created_datetime     = $now;
		$run->save();
